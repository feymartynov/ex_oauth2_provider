defmodule ExOauth2Provider.Test.QueryHelper do
  ...
  def get_last_access_grant do
    ExOauth2Provider.repo().one(from(x in OauthAccessGrant, order_by: [desc: x.id], limit: 1))
  end

  def get_last_access_token do
    ExOauth2Provider.repo().one(from(x in OauthAccessToken, order_by: [desc: x.id], limit: 1))
  end
