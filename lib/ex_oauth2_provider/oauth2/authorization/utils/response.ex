defmodule ExOauth2Provider.Authorization.Utils.Response do
  ...
  @doc false
  @spec preauthorize_response(Map.t()) ::
          {:ok, Ecto.Schema.t(), [String.t()]}
          | {:error, Map.t(), integer}
          | {:redirect, String.t()}
          | {:native_redirect, %{code: String.t()}}
          | {:native_redirect, %{access_token: String.t()}}
  def preauthorize_response(%{grant: grant} = params),
    do: build_response(params, %{code: grant.token})

  def preauthorize_response(%{access_token: access_token} = params),
    do: build_response(params, %{access_token: access_token.token})
  ...
  @doc false
  @spec authorize_response(Map.t()) ::
          {:ok, Ecto.Schema.t(), [String.t()]}
          | {:error, Map.t(), integer}
          | {:redirect, String.t()}
          | {:native_redirect, %{code: String.t()}}
          | {:native_redirect, %{access_token: String.t()}}
  def authorize_response(%{grant: grant} = params),
    do: build_response(params, %{code: grant.token})

  def authorize_response(%{access_token: access_token} = params),
    do: build_response(params, %{access_token: access_token.token})

  def authorize_response(%{error: error} = params), do: build_response(params, error)

  @doc false
  @spec deny_response(Map.t()) ::
          {:error, Map.t(), integer}
          | {:redirect, String.t()}
          | {:native_redirect, %{code: String.t()}}
          | {:native_redirect, %{access_token: String.t()}}
